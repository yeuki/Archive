...
### Clear-transmission material revision

User accepted the softer edge but found the body too foggy relative to the reference. Reduced full-body fixed softening from 3px to 0.5px and substantially lowered the existing white/dark surface wash instead of adding another layer. A shared root filter token keeps live and page-transition sampling consistent. Edge refraction retains the same displacement strength and feather mask, with 0.35px softening (CSS-only fallback 0.75px) instead of 1px/3px. Geometry, selection, touch lighting, page-overlap protection, safe areas and solid accessibility fallbacks remain unchanged. No extra dependencies, samplers, animations or data changes.

Added a controlled central-body contrast comparison against the previous frosted material on the same stripe background, hiding foreground controls only in the isolated rendering test. This complements, rather than replaces, the existing live displacement, central-body transition sampling and geometry checks. Candidate remains Unreleased; no merge, version, Drive archive or physical-phone installation.

Full `npm run verify`, Capacitor sync, Android unit tests, lint and debug assembly passed. Production-browser and packaged Android/WebView checks passed at both phone sizes, including navigation/transition ordering, full-body sampling, geometry, consistent scroll material and accessibility fallbacks. Controlled central stripe contrast increased from approximately 32 to 161 in browser captures and 30 to 160 in actual Android compositor captures; these are material transmission measurements, not app-performance claims. Edge displacement changed 1,340/1,531 browser pixels and 13,542/14,582 Android pixels, with zero changed flat-center pixels. No idle dock animation. Browser scrolling p95 was approximately 8/9ms and software-GPU emulator p95 117/100ms; physical-phone smoothness/readability remains unverified. Evidence: `test-results/optical-dock-clear-body-browser-independent`, `test-results/optical-dock-clear-body-native-360-final` and `test-results/optical-dock-clear-body-native-412`.

Rendering-test isolation matters: hide foreground controls for the controlled optical pixel probes because SVG invalidation can re-rasterize them, and test displacement before temporarily substituting the older frosted body. Mutating the sibling body filter first can invalidate Chromium's shared sample raster and contaminate an otherwise independent center-pixel comparison. No displacement thresholds were weakened. Reused native WebViews begin at scroll top for the full-navigation geometry precondition; compact behavior remains tested during scrolling. The preview revision is saved only to its existing feature branch; no phone or published release changes.

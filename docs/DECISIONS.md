...
- **Decision:** Replace the layered white glass with a continuous transparent optical body, 3px center softening, a feathered 7px optical perimeter, restrained interrupted directional highlights, modest shadows and a translucent semantic selection. Join straight-side and true semicircular masks instead of punching out a hard ring. Use a cached SVG channel map and tiny edge-only displacement on supporting Chromium engines, with CSS and solid accessibility fallbacks. During page switches only, temporarily fence the dock above page snapshots and sample their backdrop within the capsule. No caustic glow, absorption, rainbow outline, shader dependency or idle animation.
- **Rationale:** A 90%-opaque scrolling fill caused visible whitening. Retained dock opacity animation and a permanently named View Transition layer prevented actual backdrop sampling. Removing those idle roots enables real under-content influence. The first candidate left named page snapshots above the root-captured dock; a temporary page-only snapshot fixes that overlap without restoring permanent isolation. Gentle full-body shading and soft thickness avoid an empty outlined-glass appearance.

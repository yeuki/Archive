...
- The dock uses a continuous low-alpha optical body with a clear, lightly softened center, gentle curvature and a feathered refraction/highlight perimeter. Fade the optical edge into the body with true semicircular cap masks; do not use a hard punched-out ring, continuous bright stroke or tight dark outline. Do not make it an outlined empty capsule or a uniformly white frosted bar. Never make scrolling or expansion substitute a whiter/opaque material. Selection uses the same translucent material with a restrained semantic tint.

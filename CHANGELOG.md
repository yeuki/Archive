...
- Feathered the navigation's optical edge into the body using joined straight-side/semicircular masks; removed the hard exclusion ring, bright continuous highlight and tight dark shadow. Softened selection edging without changing navigation or the accepted page-overlap fix.
- Cleared the remaining navigation frost by reducing body blur from 3px to 0.5px, substantially lowering the white surface wash and sharpening the refracted edge. Live and transitioning glass now share the same transmission filter; accessibility fallbacks, navigation geometry and overlap protection are unchanged.
